...
from Gru_tf_noin import GRU_noin
...
def main():
    ...
    emb_vals = np.random.randn(1024,2688)
    #emb_vals = np.zeros(1024,2688)
    ...
    #embedding defintion
    inp_lkup = tf.get_variable("inp_lkup",[1024,2688],initializer = tf.constant_initializer(emb_vals))
    
    #inp_lkup = tf.get_variable("inp_lkup",[1024,2688],initializer = tf.constant_initializer(input_w))
    embed_out = tf.gather_nd(inp_lkup,prev_sample_in)
    #embed_out = tf.gather_nd(inp_lkup,prev_sample)
    RNN_in = embed_out
    RNN_in = tf.reshape(RNN_in,(1,in_dim))
    sh = tf.shape(RNN_in)

    #GRU Cell
    gru_l = GRU_noin(in_dim,hidden_dim)
    #ideally we dont need this, but adding a seperate GRU to enable easier cutting, so W params are not shared across timesteps;
    gru_2 = GRU_noin(in_dim,hidden_dim)
    ...
    gru_in_0 = tf.add(RNN_in,cond_in_0)
    ...
    RNN_in_1 = embed_out
    RNN_in_1 = tf.reshape(RNN_in_1,(1,in_dim))
    #RNN_in = gru_in_1

    #gru_in_1 = tf.concat([RNN_in_1,cond_in_0],1)
    gru_in_1 = tf.add(RNN_in_1,cond_in_0)
    ...
    with tf.Session() as session :
      ...
      tf.train.write_graph(session.graph,'.','wavernn_tf_ts1_membed_z.pbtxt')
      saver.save(session,'./wavernn_tf_ts1_membed_z')
      print(result)
