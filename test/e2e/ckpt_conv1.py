...
# Copy of trivnet_conv1 but uses TF checkpoint models instead of TF freeze graph
...
with tf.Session(config=config) as sess:
  ...
  if not outPrefix.startswith("/"):
    prefixTFfix = "./"
  tf.saved_model.simple_save(sess, prefixTFfix + outPrefix + "model", inputs={"input:0": i0}, outputs={netName+"/output" : output})
