    ...
    stages {
        ...
        stage('test_qemu') {
            steps {
                catchError {
                    sh '''
                    (cd $TEST_DIR/test_qemu_compiler && make -f $KAENA_PATH/test/e2e/Makefile check_emu_weekend 'TEST_EMU_OPTS=--force_qemu --parallel 16')
                    '''
                }
            }
            post {
                always {
                    catchError {
                        ...
                        sh 'for f in `find $TEST_DIR/test_qemu_compiler/*/working_dir  -iname "*.npy" -a -type f`; do cp $f --parents  /artifact/test_qemu/;done; '
            ...
            stages {
                stage('compiler_test') {
                    steps {
                        catchError {
                            sh '''
                            (cd $TEST_DIR/test_emu_compiler && export QEMU_KRT_NUM_INFERENCES=$NUM_INFERENCES && export KAENA_ZEBU_SERVER=$ZEBU_SERVER && make -f $KAENA_PATH/test/e2e/Makefile check_emu_weekend 'TEST_EMU_OPTS=--force_qemu --parallel 1 --timeout 3600 --cached_kelf /workdir/test/test_qemu_compiler')
                            '''
                        }
                    }
                    post {
