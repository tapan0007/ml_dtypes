...
Sequencer::Sequencer() : row_countdown(0), column_countdown(0), psum_id(0), psum_dtype(UINT8), psum_start(false), psum_end(0),  ifmap_valid(false), ifmap_addr(0), ifmap_stride(0), weight_valid(false), weight_addr(0), weight_stride(0), weight_dtype(UINT8), toggle_weight(false), clamp(false), clock(0) {
}
...
void
Sequencer::set_psum_start(bool truth) {
    psum_start = truth;
}

void
Sequencer::set_psum_end(bool truth) {
    psum_end = truth;
}

void
Sequencer::set_psum_id(int id) {
    psum_id = id;
}
...
Sequencer::pull_edge() {
    EdgeSignals es = EdgeSignals{.row_countdown = row_countdown, .column_countdown = column_countdown, \
        .ifmap_valid = ifmap_valid, .ifmap_addr = ifmap_addr, .ifmap_stride = ifmap_stride, .weight_valid = weight_valid, .weight_addr = weight_addr, .weight_stride = weight_stride, weight_dtype = weight_dtype, .toggle_weight = toggle_weight, .clamp_weights = clamp,\
            .psum_id = psum_id, .psum_dtype = psum_dtype, .psum_start = psum_start, .psum_end = psum_end};
    return es;
